...
> **Prerequisites (iOS):**
> 1. **Xcode — check the version first, then match the Couchbase Lite SDK to it.** **Xcode 26.2+ is recommended** because it gets the latest Couchbase Lite **4.x** (currently 4.1) — the version we encourage for new apps, and the one future peer-to-peer/Bluetooth support needs. **Xcode 16.x is supported** — 16.3+ also gets 4.x (4.1.x loads on Swift 6.1+); only 16.0–16.2 needs to pin to **4.0.x** instead (4.1.x needs Swift 6.1+ and won't load there: `SDK not supported by the compiler`). There's no reason to use the 3.x line on any current Xcode. Check: Xcode → menu → About Xcode.
...
2. **If it's older than 26.2** (e.g. 16.x): **strongly recommend upgrading Xcode** so they get the latest Couchbase Lite 4.x (recommended for new apps; required for future P2P/Bluetooth). Point them to the **Mac App Store** (simplest) or https://developer.apple.com/download/applications/ for a specific version; the latest Xcode may need a recent macOS. If they can't/won't upgrade: **16.3+ still gets 4.x** (4.1.x); only **16.0–16.2** needs to stay on **4.0.x** — confirm they're OK proceeding on that line.
3. **Pin the SDK to the newest version compatible with the environment** — repo is always `couchbase-lite-swift-ee.git` (no separate `-ios-ee` repo, any Xcode). Xcode 26.2+ or 16.3–16.4 → CBL 4.x latest, currently **4.1.0** (`upToNextMajor`); Xcode 16.0–16.2 → **4.0.x**, currently **4.0.4** (`upToNextMinor` — 4.1.x needs Swift 6.1+). Details in `couchbase-lite-ios-app` → `reference/installation-and-plist.md`. Never pin `upToNextMajor` from a 4.0.x on Xcode 16.0–16.2 — it slides to 4.1.x and fails to load.
...
> 1. **Android Studio** (a current release — Giraffe/Koala or newer; a codename you don't recognize means newer, not older — never treat an unfamiliar/unlisted version name as "too old", see step 2 below). It bundles **JDK 17** and a compatible **AGP 8.x / Gradle 8.x**, which Couchbase Lite Android **4.x** (currently 4.1) needs — and which every current 3.x release (3.3.3+) needs too, so there's no older-toolchain fallback version to pin instead. Check: Android Studio → About. If the toolchain can't run AGP 8 / JDK 17, the fix is to update Android Studio, not to pin an older CBL version.
...
2. **Deciding "old enough to need an upgrade" — do NOT check the name against a fixed list of codenames.** Android Studio's codename alphabet (…Giraffe, Koala, Ladybug, Meerkat, Narwhal, Otter, Panda…) keeps advancing roughly every 6 months, so any list baked into this skill — or improvised on the spot, including via a multiple-choice question — goes stale and will eventually reject a perfectly current release just because its name is newer than what's known. **A codename you don't recognize is a sign it's newer than your own knowledge, not that it's old** — treat any named codename (recognized or not) as current and proceed; only flag "needs upgrade" when the user reports an actual old version *number* (roughly pre-2022, i.e. before Android Studio's rebrand to codenames) or says outright they can't get Gradle sync to run on AGP 8 / JDK 17. If you're unsure, ask a plain yes/no ("can you open the project and get a successful Gradle sync?") rather than asking them to pick their version from a list you provide — a free-text answer or **About** screen is all this needs. **Updating Android Studio is required, not optional** only for a genuinely old, pre-Giraffe release that can't run AGP 8 / JDK 17 — every current CBL release (4.x and 3.3.3+/3.4.x alike) needs it, so there's no older-toolchain line to fall back to. Point them to Help → Check for Updates, or the Android Studio download page.
3. **Ask whether the app must run on Android 5.1/6.0 (API 22/23) devices.** Default: **pin the latest CBL 4.x** (`couchbase-lite-android-ee-ktx`, Enterprise Edition, from the custom Couchbase Maven repo), minSdk 24 — AGP 8 / JDK 17 required either way (there's no older-toolchain fallback line). **If yes** (older devices in the field): pin **CBL 3.2.4** instead, minSdk 22 — a real, supported fallback, but it means the client-generation step writes the *older* Replicator-setup form (`ReplicatorConfiguration(endpoint)` + `addCollection`, not the 4.x constructor form). Carry this choice into step 5 (client generation) so `couchbase-lite-android-app` generates the matching code — see its `reference/installation-and-config.md` and `reference/cbl-kotlin-apis.md`. This does **not** relax the AGP 8 / JDK 17 toolchain requirement, only the app's own minSdk.
4. **Set the Gradle JDK to 17-22** (before the first Gradle sync) -- new Android Studio may default it to JDK 24/25, which the bundled **Gradle 8.9** wrapper can't run (Gradle 8.9 only runs on JDK 17-22; 23 needs Gradle 8.10+, 24 needs 8.14+, 25 needs 9.1.0+); this is the #1 first-build failure. **Walk the user through it in your reply -- give these exact steps, do NOT just say "set it to 17" or defer to the README:**
   - **Most likely, right when the project is first opened -- before any sync error:** Android Studio shows a modal titled **"Please Select Gradle JVM to Import Project"** with a **"Use JVM `<N>`"** button. Tell them to **just click "Use JVM `<N>`"** -- Android Studio only ever offers a compatible JDK there, so it's always safe to take, one click, done.
   - **Otherwise, if a sync already failed** with "Incompatible Gradle JVM version" / "Gradle 8.9 requires Java …": click the **"Apply compatible Gradle JDK configuration and sync"** link shown right in the sync error -- Android Studio downloads a compatible JDK (e.g. 21) and re-syncs automatically.
   - **Manual fallback:** in Android Studio open **Settings/Preferences -> Build, Execution, Deployment -> Build Tools -> Gradle -> Gradle JDK** (the current official name and location, per developer.android.com/build/jdks -- NOT "Default Gradle JVM criteria", an older/incorrect label some instructions still use, and NOT "Gradle Daemon JVM criteria", a genuinely different, newer feature that needs Gradle 9.2+ and never applies here since this skill pins Gradle 8.x); pick the embedded **JDK / JetBrains Runtime (`jbr-…`)** entry, or the **`GRADLE_LOCAL_JAVA_HOME`** macro (Google's own default for new projects -- normal to see, not a different setting), resolved to **17 through 22** -- avoid a resolved path pointing at a standalone JDK 23/24/25; click **Apply / OK**; then **File -> Sync Project with Gradle Files**.
   (Also captured in the generated README and `couchbase-lite-android-app`'s `reference/installation-and-config.md` / `reference/gradle-project-generation.md`.)
...
### The access-pattern question (step 4)
...
7. **Offer seed/demo data — MANDATORY, do not skip.** Once the endpoint is Online you **must ask** (as its own one-question prompt) whether the user wants a couple of demo documents so the two-simulator test shows data immediately (e.g. one record assigned to `bob` + one shared reference). This is easy to skip when provisioning ran in the background — don't. Only create on a yes; announce exactly what you're writing, label test/verification docs (`zz-test-…`), and say how to remove them. Never write documents to their backend silently. **On a yes, run the writes yourself right away — do not hand the user a list of curl commands to run themselves.** Unlike `setup-capella.sh` (deliberately user-run — long-running, needs their own API key), seeding is quick and uses credentials the script already generated; see `couchbase-appservices-provisioning` → "Seed / test data" for why these are different.
...
- **Android:** Run — open the ROOT project folder (File → Open → the `<AppName>` folder with `settings.gradle.kts`) in Android Studio; **set the Gradle JDK to 17-22** (one-time; new Android Studio may default it to JDK 24/25, which the bundled Gradle 8.9 wrapper cannot run) -- easiest is clicking **"Use JVM `<N>`"** on the "Please Select Gradle JVM to Import Project" dialog if it appears, or **"Apply compatible Gradle JDK configuration and sync"** on a sync error; manually: Settings/Preferences -> Build, Execution, Deployment -> Build Tools -> Gradle -> **Gradle JDK**, set it to the embedded JBR or `GRADLE_LOCAL_JAVA_HOME` resolved to **17-22** (not "Default Gradle JVM criteria" -- see step 4 above for the full explanation) and Apply; then Gradle sync, pick/create an emulator, Run ▶ (re-sync Gradle after the script writes `local.properties`). Two-emulator test — start both emulators, then in the device dropdown click **Select Multiple Devices…**, tick both, **Run ▶ once** to deploy to all at once (or run one target at a time); sign in independently (manager on one, bob on the other — the app always starts at Login, storage is per-emulator). Use two SEPARATE AVDs, not a Duplicate. Mirror `couchbase-lite-android-app/reference/testing-offline-sync.md`.
...
2. **Backend -- the user runs it themselves** (Claude never runs it; the API key stays on their machine). Present the steps as a **numbered list in the order performed** -- create the key in the Capella UI *first* (it's not in the project folder), *then* edit the file, *then* run:
     ...
     2. **Open `provision.env`** in the `<project>/` folder and:
        ...
        - leave `APPSVC_ADMIN_PASS` blank (recommended) — the script generates and saves a strong password on first run, never printed; set it yourself here instead only if you want a specific value
        - `MANAGER_PASS`/`BOB_PASS` are pre-filled with the static default `Password1!` — fine for low-privilege test accounts; edit those two lines directly if you want different values
        - (app values — endpoint, collections, `SYNC_FUNCTIONS_DIR` — are already filled in)
     3. **Run:** `source provision.env && ./setup-capella.sh` (20–45 min; writes the WSS URL into the client config — iOS `Info.plist` / Android `local.properties`; creates the `manager`/`bob` App User logins — default password `Password1!` for both, overridable via `MANAGER_PASS`/`BOB_PASS` in `provision.env`).
     4. **Tell me when the App Endpoint is Online.** (The script creates default App User logins `manager` / `Password1!` (admin role) and `bob` / `Password1!` (regular) — check the script's own printed summary for the actual credentials used, since these are overridable via `MANAGER_PASS`/`BOB_PASS`.)
...
**When you give the user a literal `cd` command to their project folder, never wrap a leading `~` inside the quotes** (e.g. `cd "~/projects/.../AppName"`) -- quoting disables tilde expansion in bash/zsh, so that command fails even though `cd ~/projects/.../AppName` (no quotes) or `cd "$HOME/projects/.../AppName"` (quoted, `$HOME` not `~`) both work. Paths on this team routinely contain spaces (e.g. `mobile appdev`), so quoting is still required -- just quote `$HOME/...` or the fully expanded absolute path, never a quoted `~/...`.
