...
- `reference/role-channel-rules.md` — **critical bug-learnings**: `requireRole()` does NOT take a `role:` prefix in App Services ACFs (unlike self-managed Sync Gateway) — use `requireRole("admin")`; `access()`, by contrast, likely DOES support a `role:`-prefixed principal per Couchbase's own docs (unconfirmed against a live App Services endpoint) — this skill still grants admin channel access statically via the Admin REST API rather than a per-write `access()` call, by design, not because `access()` is assumed incapable of it; why flat `admin_channels` is silently ignored for named scopes; how role→channel access is inherited via `admin_roles`; how to set `collection_access` per scope/collection.
