...
10. **`@Composable` toolbars/scaffolds** — use Material 3 `TopAppBar` inside `Scaffold`; the sync-status indicator (color by state) and the settings action live in the app bar's `actions`. Turn the settings icon red on sync error so problems are visible without opening Settings.
...
android {
    ...
    defaultConfig {
        ...
        minSdk = 24; targetSdk = 35   // CBL 4.x -ktx AAR declares minSdk 24; project must be >= 24
...
> 3. **Fix the Gradle JVM** (the #1 first-build gotcha). New Android Studio (2025+) defaults the Gradle JVM to a too-new JDK (24/25) that the bundled **Gradle 8.9** wrapper cannot run, so the first sync fails with **"Incompatible Gradle JVM version" / "Gradle 8.9 requires Java ..."**.
>    - **Easiest — one click:** on that sync error, click **"Apply compatible Gradle JDK configuration and sync"**. Android Studio downloads a compatible JDK (e.g. 21) and re-syncs automatically. Done.
>    - **Manual alternative:** **Settings/Preferences → Build, Execution, Deployment → Build Tools → Gradle → Gradle JVM** (or **Default Gradle JVM criteria → Version** in 2025+ UIs) → choose the **Embedded JDK / JetBrains Runtime (jbr-…)** bundled with Android Studio — **17 or 21 both work**; avoid a standalone **JDK 23/24/25** → **Apply / OK** → **File → Sync Project with Gradle Files**.
>
>    (Gradle 8.9 supports **JDK 17–22**. Do this step BEFORE the first sync to skip the error entirely — but if you already hit it, the one-click link above is the fastest recovery.)
>
...
> **If sync fails with "Incompatible Gradle JVM version" / "Gradle 8.9 requires Java …" / "Unsupported class file major version" / invalid `org.gradle.java.home`:** Android Studio is running Gradle on too-new a JDK (commonly 24/25). **Fastest fix: click the "Apply compatible Gradle JDK configuration and sync" link shown right in the sync error** — Android Studio downloads a compatible JDK (e.g. 21) and re-syncs for you. Otherwise set it manually: **Settings → Build, Execution, Deployment → Build Tools → Gradle → Gradle JVM** → the **Embedded JDK / JetBrains Runtime (jbr-…)** (17 or 21) → **File → Sync Project with Gradle Files**. The bundled Gradle 8.9 wrapper supports **JDK 17–22** only; JDK 25 needs Gradle 9.1+.
