...
In {sqlpp}, a subquery is a SELECT query that is a constituent part of another {sqlpp} query or subquery.
...
* A subquery in {sqlpp} is limited to a [SELECT](n1ql-language-reference/select-syntax.adoc) query, whereas the outermost parent query can be any DML statement such as SELECT, INSERT, UPDATE, DELETE or MERGE.
* Subquery **must** be enclosed in parenthesis.
* When subquery is used in `FROM` clause, it must be aliased.
* Subqueries are {sqlpp} expressions and {sqlpp} evaluates them like any other {sqlpp} [language expressions](n1ql-language-reference/index.adoc#N1QL_Expressions).
...
See [Correlated Subqueries](n1ql-language-reference/correlated-subqueries.adoc) for more details and [Examples](n1ql-language-reference/subquery-examples.adoc).

## FROM clause in Subqueries

### Keyspace Identifier versus Expression

As described in the [FROM clause](n1ql-language-reference/from.adoc), the from-term can be a keyspace name or identifier or a {sqlpp} expression:

* Keyspace identifiers are independent sources of data for a query.
* {sqlpp} [expressions](n1ql-language-reference/index.adoc#N1QL_Expressions) can be constructed using various {sqlpp} language constructs including subqueries.
...
As mentioned in the [FROM clause](n1ql-language-reference/from.adoc), the `from-term` of both parent and subqueries allow nested path expressions over constants and subqueries.
However, only subqueries allow variable expressions (including paths), that are referenced through any [variables defined in scope](#variables-in-scope-of-a-subquery) of the subquery.
...
<a name="Q8"></a>**Q8: Find airports that are at altitudes more than 4000ft**
...
<a name="Q8A"></a>**Q8A: Q8 rewritten to use nested path using outer query variable t1**
...
The following query demonstrates the power of using nested path expressions in [correlated subqueries](n1ql-language-reference/correlated-subqueries.adoc) over the array subdocument `hotel.reviews`.
