...
* All date and time values associated with the time series are stored as integers, representing the number of milliseconds since the [Unix epoch](https://en.wikipedia.org/wiki/Unix_time).
The maximum date and time is [MaxInt64](https://docs.gtk.org/glib/const.MAXINT64.html) milliseconds -- that is, `(2^63)-1`, or approximately 292.5 million years.
...
| --- | --- | --- |
| ***ts_start***<br> _Required_ | The start date and time of the data in this document. You can use a different name for this field, but you must then specify the path to the field when you query the data. The start date and time is usually the same as the first time point in the document. However, this depends on the data, and your data storage strategy. For example, if each time series document contains a month’s data, then the start date and time may be exactly at the start of the month, rather than the first time point in the document. If this field is omitted, the document is ignored by time series queries. | Integer (milliseconds since the [Unix epoch](https://en.wikipedia.org/wiki/Unix_time)) |
| ***ts_end***<br> _Required_ | The end date and time of the data in this document. You can use a different name for this field, but you must then specify the path to the field when you query the data. The end date and time is usually the same as the last time point in the document. However, this depends on the data, and your data storage strategy. For example, if each time series document contains a month’s data, then the end date and time may be exactly at the end of the month, rather than the last time point in the document. If this field is omitted, the document is ignored by time series queries. | Integer (milliseconds since the [Unix epoch](https://en.wikipedia.org/wiki/Unix_time)) |
| ***ts_interval***<br> _Required_ for regular time series | The interval between data points, in milliseconds. You can use a different name for this field, but you must then specify the path to the field when you query the data. If this field is omitted, the document is assumed to contain an irregular time series. | Integer (milliseconds) |
| ***ts_data***<br> _Required_ | The time series data. This field must be an array. Each element in the array represents a single time point. For a regular time series, each element may be a literal representing a single value at that time point, or a nested array containing multiple values for that time point. For an irregular time series, each element must be a nested array. The first value in the nested array represents the time at that time point, in milliseconds since the [Unix epoch](https://en.wikipedia.org/wiki/Unix_time). The other values in the nested array represent the data values at that time point. If this field is omitted, the document is ignored by time series queries. | Array, or array of arrays |
...
To expand on this: if most of your queries use ranges of 2 to 4 hours, storing your time series data in documents which contain a day’s data can have an overhead of 80–90% data discard per document.
...
To minimize your storage requirements, you can set the [expiration](learn:data/expiration.adoc) for your time series documents.
...
1. Import the raw dataset from a supported format: CSV or JSON.
To do this, you can use the [cbimport](tools:cbimport.adoc) command line tool, the [import](manage:import-documents/import-documents.adoc) feature in the Couchbase UI, or an SDK data parsing library.
2. When the data is imported, transform the imported data to one or more documents with the [Couchbase time series document format](#structure-of-time-series-documents).
To do this, use an [INSERT SELECT](n1ql-language-reference/insert.adoc) query or an SDK insert operation.
   * Convert any dates and times to milliseconds since the [Unix epoch](https://en.wikipedia.org/wiki/Unix_time).
   To do this, use {sqlpp} [date-time functions](n1ql-language-reference/datefun.adoc), or date-time functions at the application level.
...
To do this, use an [INSERT SELECT](n1ql-language-reference/insert.adoc) query or an SDK insert operation, just as you did with the initial data.
* If the new data falls within the date range of an existing document, update an existing time series document.
There are two ways to do this:
  * Use an [UPSERT SELECT](n1ql-language-reference/upsert.adoc) query or an SDK upsert operation to replace an existing time series document.
  * Use an [UPDATE](n1ql-language-reference/update.adoc) query or an array-append SDK call using the sub-document API to append the new data to an existing time series document.
...
For more information, see [Time Series Data Storage Strategy](#time-series-data-storage-strategy).
...
For full details and examples, see [n1ql-language-reference/timeseries.adoc](n1ql-language-reference/timeseries.adoc).

## Examples

For these examples, use the following links to download raw time series data to your local system.

* [regular-time-series.csv, window=_blank]({attachmentsdir}/regular-time-series.csv) -- invented temperature data
* [irregular-time-series.csv, window=_blank]({attachmentsdir}/irregular-time-series.csv) -- historical house price data
footnote:ogl[]

<a name="ex-import"></a>**Import time series data from CSV files**

Use the [collection-manage](cli:cbcli/couchbase-cli-collection-manage.adoc) tool to create a scope for the time series data.
...
Use the [collection-manage](cli:cbcli/couchbase-cli-collection-manage.adoc) tool to create collections for the raw time series data within the new scope.
...
Use the [cbimport](tools:cbimport.adoc) tool to import the time series data.
...
<a name="ex-regular-insert"></a>**Convert regular time series data to a time series document**
...
<a name="ex-irregular-insert"></a>**Convert irregular time series data to a time series document**
...
<a name="ex-index"></a>**Create indexes for time series data**

For this example, set the query context to the `time` scope in the travel sample dataset.
For more information, see [Query Context](n1ql:n1ql-intro/queriesandresults.adoc#query-context).

The following query creates an index for the time series data created in [Convert regular time series data to a time series document](#ex-regular-insert).

```sqlpp
CREATE INDEX idx_mean_temp ON weather(region, ts_end, ts_start);
```

The following query creates an index for the time series data created in [Convert irregular time series data to a time series document](#ex-irregular-insert).

```sqlpp
CREATE INDEX idx_sales_prices ON housing(district, ts_end, ts_start);
```

<a name="ex-update"></a>**Add time series data to an existing time series document**
...
The newly imported data is converted by a subquery, which aggregates the mean temperature figures into a single time series data array, as in [Convert regular time series data to a time series document](#ex-regular-insert).

The query sets the end date and time for the time series to the end of the year 2013.
See [Update regular time series end date and time](#ex-regular-end) and [Update irregular time series end date and time](#ex-irregular-end) for other ways to set the end date and time for the time series.

The query specifies that the updated time series document should keep its current time-to-live.
Note that it is also possible to preserve the document time-to-live using the request-level [preserve_expiry](n1ql:n1ql-manage/query-settings.adoc#preserve_expiration}) parameter.

<a name="ex-regular-end"></a>**Update regular time series end date and time**
...
<a name="ex-irregular-end"></a>**Update irregular time series end date and time**
...
* Querying time series data: [n1ql-language-reference/timeseries.adoc](n1ql-language-reference/timeseries.adoc)
* How-to guide: [guides:import.adoc](guides:import.adoc)
