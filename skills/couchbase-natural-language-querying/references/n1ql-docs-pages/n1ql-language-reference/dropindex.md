...
The DROP INDEX statement allows you to drop a secondary index, a Composite Vector index, or a Hyperscale Vector index.
Dropping an index that has replicas will drop all of the replica indexes too.

The [DROP VECTOR INDEX](n1ql-language-reference/dropvectorindex.adoc) statement is a synonym for the DROP INDEX statement.
...
To use the DROP INDEX statement, you must have the `Query Manage Index` privilege on the keyspace or bucket.
...
![Syntax diagram: see source code listing](../../assets/images/n1ql-language-reference/drop-index.png)
...
<a name="ex-1"></a>**Drop index from the default collection in the default scope**
...
<a name="ex-2"></a>**Drop index from a named collection with path**
...
<a name="ex-3"></a>**Drop index from a named collection with query context**
