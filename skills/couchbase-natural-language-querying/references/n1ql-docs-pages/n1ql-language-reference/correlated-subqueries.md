...
For instance, in examples [Q1](n1ql-language-reference/subqueries.adoc#Q1), [Q2](n1ql-language-reference/subqueries.adoc#Q2), and [Q3](n1ql-language-reference/subqueries.adoc#Q3) on the Subqueries page, the subquery has no correlation with its outer query.
...
See [Nested Paths in Subqueries](n1ql-language-reference/subqueries.adoc#nested-path-expr) for more details.
* **Better Performance**\
...
This significantly improves the performance of such subqueries, as shown in example [Q6](n1ql-language-reference/subqueries.adoc#Q6) on the Subqueries page.
In contrast, example [Q6A](n1ql-language-reference/subqueries.adoc#Q6A) on the Subqueries page cannot take advantage of this optimization.

### Correlation by Reference (or non FROM clause-Expression)

In this case the subquery refers to [variables in the scope](n1ql-language-reference/subqueries.adoc#section_onz_3tj_mz) of the outer level query, in clauses other than the FROM clause of the subquery.
...
For more information, see [USE KEYS in the Subquery](#from-clause-and-use-keys-in-correlated-subqueries) and [FROM clause in Subqueries](n1ql-language-reference/subqueries.adoc#from-clause).
...
**📌 NOTE**\
When a keyspace name identifier is used in the FROM clause of a subquery, it refers to a collection of documents referenced by the keyspace identifier.
...
* Similarly, the subquery in [Q9A](n1ql-language-reference/subqueries.adoc#Q9A) on the Subqueries page has an independent keyspace identifier `airport` in the FROM clause, but the correlation is self-referencing to the same document.
Therefore, `USE KEYS meta(t).id` is used.

This is exactly same as the Primary Key-Foreign Key relationship required to [join](n1ql-language-reference/join.adoc) two documents that are referenced in the outer and inner queries.
Note that in the `travel-sample` data model, the `"route"` documents refer the `"airline"` documents using the attribute `airlineid`.
Refer to the [Data Model](learn:data/document-data-model.adoc).
...
In example [Q9](n1ql-language-reference/subqueries.adoc#Q9) on the Subqueries page, the FROM clause is an expression referring to the variable alias `t` (in fact the nested path `t.reviews`) that already establishes correlation, and hence the subquery does not need an explicit USE KEYS clause.
