...
A variable that you create in one `LET` clause may be referenced in a later `LET` clause, as detailed in [Use chained LET](#ex3).

Each `LET` alias needs to be unique within its scope.

## Prerequisites

The `LET` clause can only be used in a `SELECT` statement, and in order for you to select data from a document or keyspace, you must have the `query_select` privilege on the document or keyspace.
...
* **alias**\
[Required] [Identifier](n1ql-language-reference/identifiers.adoc) that represents the name of the variable.
* **expr**\
[Required] [Expression](n1ql-language-reference/index.adoc#N1QL_Expressions) that represents the value assigned to its `alias`.
...
<a name="ex1"></a>**Use LET to set variables to a number, an operation on a field, and a subquery**
...
<a name="ex2"></a>**Use LET to set a variable in an array**
...
<a name="ex3"></a>**Use chained LET**

Variant of [Use LET to set variables to a number, an operation on a field, and a subquery](#ex1).
