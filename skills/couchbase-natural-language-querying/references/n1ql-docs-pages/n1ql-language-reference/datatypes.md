...
{sqlpp} supports many data types: MISSING, NULL, Boolean values, numeric values, string values, arrays, objects, and binary.
...
For example, "``\u003cbinary (size_in_bytes b)\u003e``", where [.var]`size_in_bytes` signifies the size of the binary data in bytes.
You can use the `TYPE()` function to retrieve the type of the value as binary.
Functions that return metadata such as `META()` still operate as expected.
...
See [ORDER BY clause](n1ql-language-reference/orderby.adoc) for more information.
...
See [Date Functions](n1ql-language-reference/datefun.adoc) for more information.
