...
{sqlpp} date functions return the system clock value or manipulate the datetime values, which are represented as a string or an integer.
...
You can read further about UTC at [https://www.timeanddate.com/time/aboututc.html](https://www.timeanddate.com/time/aboututc.html).
...
Many date functions take the time zone as an additional argument and use data from the [IANA Time Zone Database](https://www.iana.org/time-zones).
...
* **ISO-8601 Dates**
  ...
  Note, however, that if you use Go reference date values as the date components, the example date is interpreted as a [Go reference date](#date-string-formats), rather than an ISO-8601 example date.
...
* **Date String Codes**
  ...
  These are based on the alphabetic format codes from the [ISO-8601](https://www.w3.org/TR/NOTE-datetime) standard, with some extensions.
  ...
  | `YY`
  | 2-digit year ^[[note](#default-values)]^
  ...
  pass:q[Characters which are not part of the format specification are matched literally and produced unaltered, with the exception of Unicode U+0020, i.e. space `" "`, which matches any single character when parsing, and is produced unaltered on output.]
  For example, `YYYY MM DD` parses `2021-06-28`, `2021/06/28`, `2021.06.28`, etc.
* **Go Reference Dates**

  You can specify a date format using [Go language date components](https://golang.org/pkg/time/#pkg-constants).
...
* **Percent-Style Dates**
  ...
  Date and time functions also accept `printf`-style format specifiers for date formats, based on the Unix [date](https://man7.org/linux/man-pages/man1/date.1.html) command.
  ...
  ![Syntax diagram: refer to source code listing](../../assets/images/n1ql-language-reference/format-specifier.png)
  ...
  **2+| `z`**
  ...
  | When parsing, matches a time zone in any supported format -- ±hh, ±hhmm,
  ±hh:mm, zone name
  | `+01`\
    `+0100`\
    `+01:00`\
...
<dl><dt><strong><a name="default-values"></a>📌 NOTE: Default Values</strong></dt><dd>
...
<dl><dt><strong><a name="tzn-date-format"></a>📌 NOTE: TZN Date Format</strong></dt><dd>

In addition to the date formats listed [here](#date-string-formats), {sqlpp} also supports the `TZN` (Time Zone Name) format.
...
For an example of its usage, refer to the [STR_TO_TZ()](#examples) function.
</dd></dl>
...
Functions such as [DATE_ADD_STR()](#date_add_strdate1-n-part) accept a `part` argument, which is the component to adjust.
...
**❗ IMPORTANT**\
Many date functions use the local system clock value and timezone of the query node to generate results.
...
It is recommended that all Couchbase Server nodes have their [time synchronized via NTP](install:synchronize-clocks-using-ntp.adoc).
...
* **fmt**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing a [supported date format](#date-string-formats) to output the result as.
...
To avoid inconsistencies between multiple calls to `CLOCK_LOCAL()` within a single query, use [NOW_LOCAL()](#now_localfmt) instead.
...
If this function is called multiple times within the same query then the values returned may differ, particularly if the query takes a long time to run.
To avoid inconsistencies between multiple calls to `CLOCK_MILLIS()` within a single query, use [NOW_MILLIS()](#now_millis) instead.
...
If this function is called multiple times within the same query then the values returned may differ, particularly if the query takes a long time to run.
To avoid inconsistencies between multiple calls to `CLOCK_STR()` within a single query, use [NOW_STR()](#now_strfmt) instead.
...
* **tz**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [timezone](#timezones) to convert the local time to.
...
If this function is called multiple times within the same query then the values returned may differ, particularly if the query takes a long time to run.
To avoid inconsistencies between multiple calls to `CLOCK_TZ()` within a single query, use [NOW_TZ()](#now_tztz--fmt) instead.
...
If this function is called multiple times within the same query then the values returned may differ, particularly if the query takes a long time to run.
To avoid inconsistencies between multiple calls to `CLOCK_UTC()` within a single query, use [NOW_UTC()](#now_utcfmt) instead.
...
* **date1**\
An integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, representing an Epoch/UNIX timestamp in milliseconds.

  If this argument is not an integer then `null` is returned.
* **n**\
The value to increment the date component by.
This value must be an integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, and may be negative to perform date subtraction.

  If a non-integer is passed to the function then `null` is returned.
* **part**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [component](#manipulating-date-components) of the date to increment.
...
* **date1**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the date in a [supported date format](#date-string-formats).
* **n**\
The value to increment the date component by.
This value must be an integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, and may be negative to perform date subtraction.
...
* **date1**\
An integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, representing a Epoch/UNIX timestamp in milliseconds.
...
An integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, representing a Epoch/UNIX timestamp in milliseconds.
  ...
  If this argument is not an integer, then `null` is returned.
* **part**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [component](#manipulating-date-components) of the date to return.
...
* **date1**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing a date in a [supported date format](#date-string-formats).

  If this argument is not a valid date string then `null` is returned.
* **input-fmt**\
The format of the input string, `date1`.
This can be a string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string.
...
This is the value that is subtracted from `date1`.
  ...
  If this argument is not an integer, then `null` is returned.
* **part**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [component](#manipulating-date-components) of the date to extract.
...
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [component](#manipulating-date-components) of the date to increment.
  ...
  If an invalid part is passed to the function, then `null` is returned.
* **n**\
An integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, representing the value by which to increment the part component for each generated date.
...
The input dates can be in any of the [supported date formats](#date-string-formats).
...
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [component](#manipulating-date-components) of the date to increment.
  ...
  If an invalid part is passed to the function, then `null` is returned.
* **quantity_int**\
An integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, representing the value by which to increment the interval component for each generated date.
...
To ensure that both dates have the same format, you should use [DATE_FORMAT_STR()](#date_format_strdate1-input-fmt-fmt).
...
This is the date used as the date to truncate.
  ...
  If this argument is not an integer, then `null` is returned.
* **part**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [component](#manipulating-date-components) to truncate to.
...
* **duration**\
A number, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a number, which represents the duration to convert to a string.
This value is specified in nanoseconds (1×10^-9^ seconds).
...
Alias for [MILLIS_TO_STR()](#millis_to_strdate1--fmt-).
...
Alias for [MILLIS_TO_TZ()](#millis_to_tzdate1-tz--fmt)
...
If you wish to use the system time when the function is evaluated, use [CLOCK_LOCAL()](#clock_localfmt) instead.
...
If this function is called multiple times within the same query it will always return the same time.
If you wish to use the system time when the function is evaluated, use [CLOCK_MILLIS()](#clock_millis) instead.
...
* **tz**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [timezone](#timezones) to convert the query timestamp to.
...
If this function is called multiple times within the same query it will always return the same time.
If you wish to use the system time when the function is evaluated, use [CLOCK_TZ()](#clock_tztz--fmt) instead.
...
If this function is called multiple times within the same query it will always return the same time.
If you wish to use the system time when the function is evaluated, use [CLOCK_STR()](#clock_strfmt) instead.
...
If this function is called multiple times within the same query it will always return the same time.
If you wish to use the system time when the function is evaluated, use [CLOCK_MILLIS()](#clock_utcfmt) instead.
...
* **duration**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the duration to convert.
...
* **date1**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the date to convert to Epoch/UNIX milliseconds.

  If this argument is not a valid date format, then `null` is returned.
* **format**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the expected format of the input date string, using the [Go language reference date](https://golang.org/pkg/time/#pkg-constants).

  **Optional argument**.
  If not specified, the input date string must be in a [supported date format](#date-string-formats).
...
This can be a string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, and must be a [supported date format](#date-string-formats).
...
Alias for [STR_TO_TZ()](#str_to_tzdate1-tz--input-fmt-fmt).
...
An integer, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to an integer, representing an Epoch/UNIX timestamp in milliseconds.
* **tz**\
A string, or any valid [expression](n1ql-language-reference/index.adoc) which evaluates to a string, representing the [timezone](#timezones) to for the expr argument.
...
Note that his function returns the string value of the day of the week, where [DATE_PART_STR()](#date_part_strdate1-part) with part = "dow" returns an integer value of the weekday (0-6).
