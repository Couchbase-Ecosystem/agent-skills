...
The Flex Index feature enables you run a {sqlpp} query as a full-text search query, using a full-text index.
...
It is possible to perform a full-text search within a {sqlpp} query using [search functions](n1ql-language-reference/searchfun.adoc).
...
For a general introduction to creating full-text indexes, refer to [Creating Indexes](fts:fts-creating-indexes.adoc).
...
* A Flex Index query cannot use a full-text index as a [covering index](indexes:covering-indexes.adoc).
* A Flex Index query cannot use a full-text index with a query on fields whose names contain special characters.
* A Flex Index query may include LIMIT and OFFSET clauses, but they cannot be [pushed down to the full-text index](indexes:index_pushdowns.adoc).
* A Flex Index query may include Aggregate functions and Window functions, but they cannot be [pushed down to the full-text index](indexes:index_pushdowns.adoc).
...
For further details, refer to [Usage](#usage) below.
...
For further details, refer to [BUILD INDEX](n1ql-language-reference/build-index.adoc).
...
For full details, refer to [USE Clause](n1ql-language-reference/hints.adoc#index-type).
* Set the `use_fts` request-level parameter to `true`.
For full details, refer to [n1ql:n1ql-manage/query-settings.adoc](n1ql:n1ql-manage/query-settings.adoc).
...
<a name="ex-usage-1"></a>**Use FTS, no index specified**
...
<a name="ex-usage-2"></a>**Use FTS or GSI, no index specified**
...
<a name="ex-usage-3"></a>**Prefer the specified FTS and GSI indexes**
...
If the query predicate contains a search function, none of this applies -- instead, an index is selected for the query according to the method described on the [Search Functions](n1ql-language-reference/searchfun.adoc) page.
...
2. Open the **Default Analyzer** pull-down menu and select `keyword`.
   ...
   !["FTS Settings Advanced Panel with Default Analyzer set to keyword"](../../assets/images/flex-fts-keyword-analyzer.png)
...
When [creating a full-text definition in the Couchbase Web Console](fts:fts-creating-indexes.adoc#inserting-a-child-field), the child fields listed by field name or by **searchable as** may be used within a {sqlpp} query.

**📌 NOTE**\
The type field in a custom type mapped index is _not_ searchable.

<a name="ex-indexed-fields"></a>**Child fields that may be used in a query**
...
<a name="ex-conditional-1"></a>**Conditional expression for a custom type mapping with a simple predicate**
...
<a name="ex-conditional-2"></a>**Conditional expression for a custom type mapping with a complex predicate**

The following query is ambiguous, and cannot be used with the full-text index defined in [Conditional expression for a custom type mapping with a simple predicate](#ex-conditional-1):
...
With brackets setting the priority of the AND and OR operators, the following queries are unambiguous, and can be used with the full-text index defined in [Conditional expression for a custom type mapping with a simple predicate](#ex-conditional-1):
...
<a name="ex-conditional-3"></a>**Conditional expression for multiple custom type mappings**
...
<a name="ex-equality-1"></a>**Equality expressions with a dynamic keyword index**
...
<a name="ex-equality-2"></a>**Equality expressions with explicitly indexed fields**
...
<a name="ex-equality-3"></a>**Equality expression with field name**
...
<a name="ex-and"></a>**AND expressions**
...
You can use compound expressions with a full-text index, as long as they respect the rules of [AND Expressions](#and-expressions) and [OR Expressions](#or-expressions) expressions described above, and do not return false negatives.

<a name="ex-compound"></a>**Compound expressions**
...
<a name="ex-ranges"></a>**Range expressions**
...
You can use the [ISSTRING()](n1ql-language-reference/typefun.adoc#isstring) and [ISNUMBER()](n1ql-language-reference/typefun.adoc#isnumber) functions as a workaround to support open-ended ranges with a full-text index.
...
Refer to [Collation](n1ql-language-reference/datatypes.adoc#collation) for more information.

<a name="ex-isstring-and-issnumber"></a>**Workarounds for open-ended ranges**
...
If the `LIKE` expression contains a simple string, it must respect the rules outlined in the [Equality Expressions](#equality-expressions) section above.
...
<a name="ex-like"></a>**LIKE expressions**
...
The range specified by the `BETWEEN` expression must respect the rules outlined in the [Range Expressions](#range-expressions) section above.
`BETWEEN` expressions that mix data type boundaries cannot be used with a full-text index.

<a name="ex-between"></a>**BETWEEN expressions**
...
You can use an pass:q[`ANY` ... `IN` ... `SATISFIES`] expression with a full-text index.
The pass:q[`ANY` ... `IN` ... `SATISFIES`] expression must operate over an array, which may be an array of objects or any supported data types.

<a name="ex-any-in-satisfies"></a>**ANY ... IN ... SATISFIES expressions**
...
You can use an pass:q[`EVERY` ... `IN` ... `SATISFIES`] expression with a full-text index.
The pass:q[`EVERY` ... `IN` ... `SATISFIES`] expression must operate over an array, which may be an array of objects or any supported data types.

<a name="ex-every-in-satisfies"></a>**EVERY ... IN ... SATISFIES expressions**

The following predicate may be used with the full-text index defined in [ANY ... IN ... SATISFIES expressions](#ex-any-in-satisfies):
...
You can use an pass:q[`ANY AND EVERY` ... `IN` ... `SATISFIES`] expression with a full-text index.
The pass:q[`ANY AND EVERY` ... `IN` ... `SATISFIES`] expression must operate over an array, which may be an array of objects or any supported data types.

<a name="ex-any-and-every-in-satisfies"></a>**ANY AND EVERY ... IN ... SATISFIES expressions**
...
Refer to [{sqlpp} Predicates](#sqlpp-predicates) above.
