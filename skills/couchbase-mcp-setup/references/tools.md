...
Exact tool names the server exposes (v0.8). Use these for verification, and for `CB_MCP_DISABLED_TOOLS` / `CB_MCP_CONFIRMATION_REQUIRED_TOOLS` lists (names must match exactly). `[W]` = write tool, not loaded when `CB_MCP_READ_ONLY_MODE=true` (the default). See [`safety.md`](safety.md).
