...
The documentation is built with [Docusaurus](https://docusaurus.io/) and lives in a dedicated [repository](https://github.com/couchbase/mcp-server-couchbase-docs). The site is **versioned**: there is no unversioned `docs/` folder — all content lives under `versioned_docs/version-<X>/`.

## Prerequisites

- **Node.js 22** — pinned in `.nvmrc`. With `nvm` installed, run `nvm use` in the repo root. Check with `node --version`.
...
The site is automatically deployed to [GitHub Pages](https://mcp-server.couchbase.com/) via GitHub Actions on every push to `main`. See `.github/workflows/deploy-docs.yml` in the docs repo.

PR previews are also deployed automatically — a preview URL is posted as a comment on each PR.
