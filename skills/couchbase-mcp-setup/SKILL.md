...
- `CB_MCP_READ_ONLY_MODE=true` blocks all writes — KV mutations and data-modifying SQL++ (write tools are not even loaded). **Keep it `true`** for exploration and for safety — most skills only read. The bundled `mcp.json` defaults it to `true`, but the server's *own* default is `false` on `1.0+`, so on the `claude mcp add` / direct-config routes **set it explicitly** rather than relying on the default.
...
- Set `CB_MCP_READ_ONLY_MODE=false` only when the user explicitly chose read-write access above — and confirm once more before generating it. `CB_MCP_READ_ONLY_MODE` is the **single write switch** — this one variable governs both KV and query writes.
