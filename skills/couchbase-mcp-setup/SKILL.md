...
- `CB_MCP_READ_ONLY_MODE=true` blocks all writes — KV mutations and data-modifying SQL++ (write tools are not even loaded). **Keep it `true`** for exploration and for safety — most skills only read. The bundled `mcp.json` defaults it to `true`, but the server's *own* default flipped from `true` (`0.8.x`) to `false` (`1.0+`), so on the `claude mcp add` / direct-config routes **set it explicitly** rather than relying on the default.
...
Pass `CB_MCP_READ_ONLY_MODE` **explicitly** (as above) on this and the other direct-config routes — don't rely on the server default, which is `true` on `0.8.x` but flips to `false` on `1.0+` (the version range spans both, so an omitted flag could silently enable writes). To enable writes, pass `-e CB_MCP_READ_ONLY_MODE="false"` (and on `0.8.x` also `-e CB_MCP_READ_ONLY_QUERY_MODE="false"` to clear the deprecated query-write block; `1.0+` has removed that flag and ignores it, so passing it stays harmless). Use `--scope user` only to share this cluster across *all* your Claude Code projects; avoid `--scope project`, which writes the credentials into a committed `.mcp.json`.
