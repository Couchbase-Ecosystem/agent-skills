...
> **Figure — Query workflow without a covering index:** The eight-step flow for a non-covered query (such as `SELECT *`). The client and **Query Service** sit at the top, with the **Index Service** on the left and the **Data Service** on the right: (1) submit the query over the REST API; (2) parse, analyze, and create the plan; (3) send a scan request with index filters to the Index Service; (4) get qualified doc keys & index keys back from the Index Service; (5) send a fetch request with doc keys to the Data Service; (6) fetch the documents from the Data Service; (7) evaluate documents into results; (8) return the query result. The defining feature is steps 5–6: a round-trip to the Data Service is required to fetch the documents.
...
> **Figure — Query workflow with a covering index:** The same flow as above but reduced to six steps because the index *covers* the query, so the **Data Service is not involved at all**. The client and **Query Service** are at the top with only the **Index Service** on the left: (1) submit the query over the REST API; (2) parse, analyze, and create the plan; (3) send a scan request with index filters to the Index Service; (4) get qualified doc keys & index keys back from the Index Service; (5) evaluate documents into results; (6) return the query result. Compared with the previous figure, the Data Service fetch (the old steps 5–6) is eliminated, which is the source of the performance improvement.
...
> **Figure — Index Service architecture:** An architecture diagram divided into three columns: a **Data Service Node**, an **Index Service Node**, and a **Query Service Node**. On the Data Service Node, two buckets (*bucket 1* and *bucket 2*) feed their mutations via **DCP streaming** into a **Projector and Router** component. The Projector and Router forwards those mutations to the **Supervisor** process on the Index Service Node, which manages the individual indexes (shown as *Index 1* through *Index 6*). The **Query Service** on the Query Service Node also communicates with the Supervisor. This illustrates that, although indexing largely lives in the Index Service, the Projector and Router components that feed it actually reside within the Data Service.
